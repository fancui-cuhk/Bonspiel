...
QueryYCSB::QueryYCSB(int is_mp)
    : QueryBase()
{
    _requests = new RequestYCSB[g_req_per_query];
    gen_requests(is_mp);
...
QueryYCSB::QueryYCSB(RequestYCSB * requests, uint32_t num_requests)
{
    ...
    for (uint32_t i = 0; i < _request_cnt; i++) {
        if (GET_WORKLOAD->key_to_part(requests[i].key) != g_part_id) {
            _is_multi_partition = true;
            break;
        }
    }
...
void QueryYCSB::gen_requests(int is_mp) {
    _request_cnt = 0;
    ...
    bool is_dist_txn;
    if (is_mp == -1)
        is_dist_txn = (g_num_parts > 1) ? (glob_manager->rand_double() < g_perc_dist_txn) : false;
    else
        is_dist_txn = is_mp;
    assert(is_dist_txn == 0 || is_dist_txn == 1);

    for (uint32_t tmp = 0; tmp < g_req_per_query; tmp ++) {
        RequestYCSB * req = &_requests[_request_cnt];
        // bool remote = (g_num_parts > 1) ? (glob_manager->rand_double() < g_perc_remote) : false;
        bool remote = is_dist_txn ? (glob_manager->rand_double() < g_perc_remote_dist_txn) : false;

        uint32_t part_id;
        if (remote) {
            _is_multi_partition = true;
            part_id = (g_part_id + glob_manager->rand_uint64(1, g_num_parts - 1)) % g_num_parts;
            has_remote = true;
        } else
            part_id = g_part_id;
    #if SINGLE_PART_ONLY
    ...
    #endif
        uint64_t primary_key = row_id * g_num_parts + part_id;
        ...
        if(req->rtype == WR)
        {
            ...
            primary_key = row_id * g_num_parts + node_id;
        }
        #endif
