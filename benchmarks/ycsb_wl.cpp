...
uint32_t
WorkloadYCSB::key_to_part(uint64_t key, uint32_t table_id)
{
...
#if HOTNESS_ENABLE
void
WorkloadYCSB::prepare_hotness_table()
{
    uint32_t num_table = 1;
    vector<uint64_t> num_row { g_synth_table_size };
...
void * WorkloadYCSB::init_table_slice() {
    ...
    {
        row_t * new_row = NULL;
        rc = the_table->get_new_row(new_row, g_part_id);
        assert(rc == RCOK);
...
QueryBase *
WorkloadYCSB::gen_query(int is_mp)
{
    QueryBase * query = new QueryYCSB(is_mp);
    return query;
}
...
{
    QueryYCSB * query = new QueryYCSB(data);
    return query;
}
