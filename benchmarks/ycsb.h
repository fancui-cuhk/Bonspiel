...
public :
    ...
    INDEX *          get_primary_index(uint32_t table_id) { return the_index; }
    table_t *        get_table(uint32_t table_id) { return the_table; };
    void             table_to_indexes(uint32_t table_id, set<INDEX *> * indexes);

    uint32_t         key_to_part(uint64_t key, uint32_t table_id = 0);
...
private:
    ...
    static void *    threadInitTable(void * This) { ((WorkloadYCSB *)This)->init_table_slice(); return NULL; }
