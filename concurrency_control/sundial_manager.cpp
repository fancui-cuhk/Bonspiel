...
SundialManager::SundialManager(TxnManager * txn, uint64_t ts)
    : CCManager(txn, ts)
{
    _is_read_only = true;
    _min_commit_ts = 0;
...
void
SundialManager::cleanup(RC rc)
{
    for (AccessSundial & access : _access_set) {
    ...
    for (AccessSundial & access : _remote_set) {
        if (access.local_data)
            delete[] access.local_data;
        access.local_data = NULL;
    }
    ...
    for (IndexAccessSundial & access : _index_access_set) {
        assert(access.type != NA);
        if (access.type != RD)
...
RC
SundialManager::get_row_permission(Access * raw_access, access_t type)
{
    RC rc = RCOK;
    assert(type == RD || type == WR);
    AccessSundial * access = (AccessSundial*) raw_access;
    ...
    if (type == RD && access->granted_access != RD) {
        ...
        return RCOK;
    } else if (type == WR && access->granted_access != WR) {
        // take write locks during execution
...
RC
SundialManager::get_row(row_t * row, access_t type, bool reserve)
{
    RC rc = RCOK;
    ...
    AccessSundial * access = get_row_access(type, row->get_table()->get_table_id(), row->get_primary_key(), _access_set);
    // access->row points to the original row in DB
    access->row = row;
    // take write locks during execution
    rc = get_row_permission(access, type);

    // if lock cannot be taken, abort
    if (rc != RCOK)
        return rc;
...
// This function upgrades the permission on raw_access->row of _txn to type
RC
SundialManager::get_index_permission(IndexAccess* raw_access, access_t type)
{
    RC rc = RCOK;
    assert(type == RD || type == WR);
    IndexAccessSundial * access = (IndexAccessSundial*) raw_access;
...
SundialManager::IndexAccessSundial *
SundialManager::get_index_access(access_t type, INDEX * index, uint64_t key, vector<IndexAccessSundial>& set)
{
    assert(type == RD || type == INS || type == DEL);
    // Search for the required row in set
    IndexAccessSundial * access = find_access(key, index->get_index_id(), set);
...
// and inserts a new entry to set (an access list)
SundialManager::AccessSundial * 
SundialManager::get_row_access(access_t type, uint32_t table_id, uint64_t primary_key, vector<AccessSundial>& set)
{
    assert(type == RD || type == WR);
    // Search for the required row in set
    AccessSundial * access = find_access(primary_key, table_id, set);
...
RC
SundialManager::index_read(INDEX * index, uint64_t key, set<row_t *> * &rows, uint32_t limit)
{
    uint64_t tt = get_sys_clock();
    IndexAccessSundial * access = get_index_access(RD, index, key, _index_access_set);
...
RC
SundialManager::index_insert(INDEX * index, uint64_t key)
{
    uint64_t tt = get_sys_clock();
...
RC
SundialManager::index_delete(INDEX * index, uint64_t key)
{
    uint64_t tt = get_sys_clock();
...
uint32_t
SundialManager::get_node_for_request(uint32_t table_id, uint64_t key, bool & reserve)
{
    uint32_t remote_part_id = GET_WORKLOAD->key_to_part(key, table_id);
    assert(remote_part_id != g_part_id);

    uint32_t remote_node_id;
    if (glob_manager->read_local()) {
...
RC
SundialManager::get_row(row_t * row, access_t type, char * &data)
{
    uint64_t tt = get_sys_clock();
...
void
SundialManager::add_remote_req_header(UnstructuredBuffer * buffer)
{
...
uint32_t
SundialManager::process_remote_req_header(UnstructuredBuffer * buffer)
{
...
void
SundialManager::get_resp_data(UnstructuredBuffer &buffer)
{
...
void
SundialManager::process_remote_resp(uint32_t node_id, uint32_t size, char * resp_data)
{
    // return data format:
    //    | n | (key, table_id, wts, rts, tuple_size, data) * n
    ...
    for (uint32_t i = 0; i < num_tuples; i++) {
        ...
        AccessSundial * access = get_row_access(type, table_id, primary_key, _remote_set);
    ...
    for (uint32_t i = 0; i < num_index_accesses; i++) {
        ...
        IndexAccessSundial * access = get_index_access(type, index, key, _index_remote_set);
        buffer.get( &access->wts );
        buffer.get( &access->rts );
        access->granted_access = RD;
        access->home_node_id = node_id;
...
LogRecord *
SundialManager::get_log_record(LogType type)
{
...
RC
SundialManager::apply_log_record(LogRecord * log)
{
    ...
    if (_state == CC_EXECUTING) {
        ...
        while (buffer.get_offset() < size) {
            ...
            if (type == IDXRD) {
                ...
                buffer.get(&table_id);
                buffer.get(&primary_key);
                AccessSundial * access = get_row_access(type, table_id, primary_key, _access_set);
                if (access->row == NULL)
    ...
    if (_state == CC_COMMITTING) {
        ...
        sort(_index_access_set.begin(), _index_access_set.end(), [](const IndexAccessSundial & a, const IndexAccessSundial & b) {
        ...
        for (; _lock_acquire_idx < _index_access_set.size(); _lock_acquire_idx++) {
            IndexAccessSundial & access = _index_access_set[_lock_acquire_idx];
            if (access.type == RD) {
        ...
        for (; _lock_acquire_idx < _index_access_set.size() + _access_set.size(); _lock_acquire_idx++) {
            AccessSundial & access = _access_set[_lock_acquire_idx - _index_access_set.size()];
...
SundialManager::AccessSundial *
SundialManager::find_access(uint64_t primary_key, uint32_t table_id, vector<AccessSundial>& set)
{
    for (auto & ac : set) {
...
SundialManager::IndexAccessSundial *
SundialManager::find_access(uint64_t key, uint32_t index_id, vector<IndexAccessSundial>& set)
{
    for (auto & ac : set) {
...
void
SundialManager::compute_commit_ts()
{
...
RC
SundialManager::process_prepare_phase_coord()
{
    RC rc = RCOK;
    // 1. compute commit ts
...
bool
SundialManager::need_prepare_req(uint32_t part_id, UnstructuredBuffer & buffer)
{
...
RC
SundialManager::process_prepare_req(uint32_t size, char * data, UnstructuredBuffer &buffer)
{
    RC rc = RCOK;
    ...
    if (_state == CC_EXECUTING) {
        ...
        for (uint32_t i = 0; i < num_index_accesses; i++) {
            ...
            buffer.get( &key );
            INDEX * index = GET_WORKLOAD->get_index(index_id);
            IndexAccessSundial * access = get_index_access(type, index, key, _index_access_set);
            buffer.get( &access->wts );
            buffer.get( &access->rts );
        }
    }

    // write set has already been locked, just validate read set
    rc = validate();
...
void
SundialManager::process_prepare_resp(RC rc, uint32_t node_id, char * data)
{
    assert(data == NULL);
}
...
void
SundialManager::process_commit_phase_coord(RC rc)
{
    if (rc == COMMIT) {
...
bool
SundialManager::need_commit_req(RC rc, uint32_t part_id, UnstructuredBuffer &buffer)
{
    return true;
}

void
SundialManager::process_commit_req(RC rc, uint32_t size, char * data)
{
    cleanup(rc);
}

bool
SundialManager::is_txn_ready()
{
    return _num_lock_waits == 0 || _signal_abort;
}

bool
SundialManager::set_txn_ready(RC rc)
{
    if (rc == RCOK)
