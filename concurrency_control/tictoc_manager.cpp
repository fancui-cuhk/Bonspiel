...
TicTocManager::get_row(row_t * row, access_t type, uint64_t key, uint64_t wts)
{
    ...
    if (!access) {
        ...
        if (type == RD || !OCC_WAW_LOCK) {
            rc = row->manager->read(_txn, local_data, ac.wts, ac.rts, true, wts != (uint64_t)-1);
            if (rc == WAIT) 
                return rc;
            _min_commit_ts = max(_min_commit_ts, ac.wts);
        }
...
TicTocManager::process_commit_phase_coord(RC rc)
{
    if (rc == COMMIT) {
        ...
        if (_txn->is_coord() && _min_commit_ts > glob_manager->get_max_cts())
