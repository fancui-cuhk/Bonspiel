...
class QueryBase {
public:
    QueryBase() { _isolation_level = SR; _is_multi_partition = false; }
    virtual ~QueryBase() {}
    ...
    bool             _is_multi_partition; /* for debug only */
#if CC_ALG == WAIT_DIE || CC_ALG == WOUND_WAIT || ((CC_ALG == TICTOC || CC_ALG == SILO || CC_ALG == DOCC) && OCC_LOCK_TYPE == WAIT_DIE)
