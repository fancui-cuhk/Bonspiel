...
public:
    ...
    virtual QueryBase *      gen_query(int is_mp = 0) = 0;
    ...
    virtual INDEX *          get_primary_index(uint32_t table_id) = 0;
    ...
    virtual uint32_t         key_to_part(uint64_t key, uint32_t table_id = 0) = 0;

#if HOTNESS_ENABLE
    virtual void             prepare_hotness_table() { assert(false); };
#endif
