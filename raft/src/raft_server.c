...
int raft_periodic(raft_server_t* me_, int msec_since_last_period)
{
    ...
    {
        /* we assume tcp connection, no need to duplicate message send
    ...
    )
{
    ...
    if (0 < ae->n_entries)
        __log(me_, node, "recvd appendentries t:%d ci:%d lc:%d pli:%d plt:%d #%d id:%d",
              ae->term,
              raft_get_current_idx(me_),
              ae->leader_commit,
              ae->prev_log_idx,
                    ...
                    msg_entry_response_t *r)
{
    ...
    for (i = 0; i < me->num_nodes; i++)
    {
         ...
         * becoming congested. */
        // todo: update next send index on leader election
        raft_index_t next_send_idx = raft_node_get_next_send_idx(node);
        if (next_send_idx == raft_get_current_idx(me_))
            raft_send_appendentries(me_, node);
    }
...
int raft_send_appendentries(raft_server_t* me_, raft_node_t* node)
{
    ...
    if (0 < me->snapshot_last_idx && next_send_idx < me->snapshot_last_idx)
    {
    ...
    ae.entries = raft_get_entries_from_idx(me_, next_send_idx, &ae.n_entries);
    ...
    if (1 < next_send_idx)
    {
        raft_entry_t* prev_ety = raft_get_entry_from_idx(me_, next_send_idx - 1);
        if (!prev_ety)
        {
        ...
        {
            ae.prev_log_idx = next_send_idx - 1;
            ae.prev_log_term = prev_ety->term;
        }
    }

    __log(me_, node, "sending appendentries node: ci:%d comi:%d t:%d lc:%d pli:%d plt:%d id:%d",
          raft_get_current_idx(me_),
    ...
    raft_node_set_next_send_idx(node, next_send_idx + ae.n_entries);
